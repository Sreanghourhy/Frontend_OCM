<template>
  <div class="ocm-float-menu-bottom fixed top-0 left-0 right-0 p-0 h-12 flex bg-blue-700" >
    ...
    <div class="absolute right-2 h-12 p-2 w-12 md:w-44 lg:w-44 xl:w-44 flex items-center justify-end cursor-pointer" @click="$router.push('/profile')" >
      <div class="ml-auto flex h-8 items-center gap-2 pl-2" >
        <div class="flex-none rounded-full overflow-hidden w-8 h-8" >
          <div v-if="profilePicture!=null" class="rounded-full w-8 h-8 flex-none mx-auto overflow-hidden bg-center bg-no-repeat bg-cover bg-white " :style=" 'background-size: 80%; background-image: url(' + profilePicture +');' " ></div>
        ...
        <div v-if="username!=''" class="hidden md:block max-w-28 h-8 leading-8 pt-1 font-moul text-left text-xs text-white truncate">{{ username }}</div>
      </div>
    </div>
    <div class="absolute left-0 h-12 p-2 w-14 sm:visible md:w-40 lg:w-40 xl:w-40 flex bg-blue-700"  >
      <div class="flex-none overflow-hidden -mt-1 w-10 h-10 " >
        <img :src="ocmLogoPng" class="h-10 mx-auto"/>
      </div>
      <div class="flex-none h-8 font-moul text-right ml-1 text-gray-700 invisible sm:visible md:visible lg:visible xl:visible" style=" white-space: nowrap; text-overflow: ellipsis;" >
        ...
        <div class="h-5 text-left text-xxs font-moul -mt-1 pt-1 text-white leading-7" >{{ $store.state.system.name }}</div>
      </div>
    </div>
