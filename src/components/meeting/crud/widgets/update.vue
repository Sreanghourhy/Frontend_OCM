            ...
            <div class=" mx-auto p-4 flex-wrap">
              <div class="crud-form-panel w-full flex flex-wrap ">
                <n-form 
                  ...
                  ref="formRef"
                >
                  <n-form-item label="កាលបរិច្ឆែក" path="date" class="w-4/5 mr-8" >
                    <officer-date-input v-model="meetingDateValue" value-type="timestamp" wrapper-class="w-full" />
                  </n-form-item>
                  <n-form-item label="កាលបរិច្ឆែក" path="year" class="w-4/5 mr-8" v-if="false" >
...
export default {
  ...
  setup(props){
    ...
    function getDatePickerTimestamp( value ){
    ...
    const meetingDateValue = ref( getDatePickerTimestamp( today.value ) )

    watch( meetingDateValue, ( value ) => {
      const selectedDate = value == null ? new Date() : new Date( value )
      meetingDateTime.year = parseInt( dateFormat( selectedDate , 'yyyy') )
      meetingDateTime.month = parseInt( dateFormat( selectedDate , 'mm') )
      meetingDateTime.day = parseInt( dateFormat( selectedDate , 'dd') )
    }, { immediate: true } )
    ...
    function clearForm(){
      ...
      meetingDateValue.value = getDatePickerTimestamp( today.value )
      ...
      props.record.date = formatMeetingDate( meetingDateValue.value )
