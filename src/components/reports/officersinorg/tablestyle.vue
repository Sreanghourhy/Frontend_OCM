<template >
    <div class="w-full p-8 relative" >
        ...
        <div v-if="selectedOrganizationRecord != null " class="gd-section w-full mb-4" >
            ...
            <div v-if="selectedOrganizationRecord?.organization" class="w-full">
                <div class="w-full flex " >
                    ...
                    <div class="cell_official_date" >ថ្ងៃខែឆ្នាំតាំងស៊ុប</div>
            ...
            <div v-for="( child , cIndex ) in organizationsList " :key="cIndex" class="tbl-border w-full border-t-0 " >
                <div :id="child.organization.keyname" v-if="selectedOrganizationRecord != null " class="tbl-caption w-full border-b border-gray-200 p-2 font-moul bg-blue-100 text-left " >{{ $toKhmer( cIndex + 1 ) + '. ' + child.organization?.name }}</div>
                ...
                <!-- <div v-for="(sp , spIndex) in ( child.structure_positions )" :key="spIndex" class="w-full flex flex-wrap" >
                    <div v-for="(oj , oIndex) in ( sp.officer_jobs )" :key="oIndex" class="w-full flex flex-wrap " >
                        <div class="cell_llr" >{{ ( cIndex + 1 ) + ( spIndex + 1 ) }}</div>
                        <div class="cell_lr" >{{ spIndex + 1 }}</div>
                        <div class="cell_code" >{{ oj.officer?.code }}</div>
                        <div class="cell_position" >{{ sp.position?.name }}</div>
                        <div class="cell_name" >{{ oj.officer?.people?.lastname + ' ' + oj.officer?.people?.firstname }}</div>
                        <div class="cell_gender" >{{ oj.officer?.people?.gender == 1 ? 'ប្រុស' : 'ស្រី' }}</div>
                        <div class="cell_dob" >{{ oj.officer?.people?.dob }}</div>
                        <div class="cell_unofficial_date" >{{ oj.officer?.unofficial_date }}</div>
                        <div class="cell_official_date" >{{ oj.officer?.official_date }}</div>
                        <div class="cell_krobkhan" >{{ oj.officer?.salary_rank  }}</div>
                        <div class="cell_certificate" >{{ oj.officer?.people?.certificates?.pop()?.field_name }}</div>
                        <div class="cell_others" >ផ្សេងៗ</div>
                    </div>
                </div> -->
            </div>
        ...
        <div v-if="selectedOrganizationRecord == null " class="place-content-center" >
            <div class="p-8 w-full mx-auto rounded bg-opacity-80 text-xl" >
...
export default {
    ...
    setup() {
        ...
        onMounted(async () => { 
            ...
            const preselectedOrganizationId = normalizeOrganizationId(route.query.organizationId)
            if( preselectedOrganizationId != null ){
                selectedOrganization.value = preselectedOrganizationId
        ...
        watch(
            ...
            (newOrganizationId) => {
                const normalized = normalizeOrganizationId(newOrganizationId)

                if( normalized == null ){
                    selectedOrganization.value = null
                    selectedOrganizationRecord.value = null
                    organizationsList.value = []
                    return
                }

                if( String(selectedOrganization.value) !== String(normalized) ){
                    selectedOrganization.value = normalized
                    updateSelectedOrganization()
                }
            }
        )
...
<style scoped >
    ...
    .cell_others {
        @apply w-24 border border-gray-100 p-1 leading-5 ;
    }
    .cell_actions {
