<template>
  <div class="absolute left-0 top-12 right-0 flex bottom-0 flex-wrap system-layout">
    <div class="flex-1 min-h-0 min-w-0 overflow-hidden flex flex-col relative">
    ...
    <!-- <float-top-menu :title="pageTitle" /> -->
    <float-top-menu :title="appBarTitle" />
    <sidebar />
  </div>
</template>
<script >
...
export default {
  ...
  computed: {
    appBarTitle() {
      ...
      const currentRouteName = matched.length
        ? matched[matched.length - 1].name
        : this.$route.name
      const tableRoutes = [
        'OfficerList',
        'OfficerListPolitical',
        'OfficerListCivilService',
        'OfficerTable',
        'OfficerProfile',
        'OfficerCreate'
      ]
      if (tableRoutes.includes(currentRouteName)) return ' '
      return undefined
    }
  }
}
