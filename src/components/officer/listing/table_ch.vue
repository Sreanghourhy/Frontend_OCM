  ...
  <!-- Top action panel of crud -->
    <div class="flex title-bar border-b border-gray-200 bg-white ">
      ...
      <div class="flex-grow action-buttons flex-row-reverse flex px-4">
        ...
        <div class="mt-1 mr-2 flex flex-wrap">
          <n-tooltip trigger="hover">
            <template #trigger>
              <div @click="$router.push('/report')" class="flex cursor-pointer hover:text-green-500 duration-300 ml-2 leading-8" >
    ...
    <div class="vcb-table-panel relative ">
      ...
      <Transition name="slide-fade" >
        <div v-if="filter" class="otc-horizontal-filters-panel ">
          ...
          <div class="filter-actions" >
            <!-- <div class="filter-action" >
              <n-tooltip trigger="hover">
                <template #trigger>
                  <div class="w-full relative" >
                    <input type="text" @keypress.enter="filterRecords(false)" v-model="table.search" class="bg-gray-100 px-1 h-9 w-full rounded border border-gray-200 focus:border-blue-600 hover:border-blue-600 duration-300" placeholder="ស្វែងរក" />
                    <svg class="absolute right-1 top-2 w-6 h-6 text-gray-400  cursor-pointer" @click="filterRecords(false)" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 20 20"><g fill="none"><path d="M8.5 3a5.5 5.5 0 0 1 4.227 9.02l4.127 4.126a.5.5 0 0 1-.638.765l-.07-.057l-4.126-4.127A5.5 5.5 0 1 1 8.5 3zm0 1a4.5 4.5 0 1 0 0 9a4.5 4.5 0 0 0 0-9z" fill="currentColor"></path></g></svg>
                  </div>
                </template>
                សូមបញ្ចូលពាក្យគន្លឹះដើម្បីស្វែងរក
            ...
            <div class="filter-action" >
              <n-select v-model:value="selectedRanks" filterable clearable multiple @update:value="filterRecords(false)" placeholder="សូមជ្រើសរើស ក្របខ័ណ្ឌ" :options="optionRanks" />
            </div>
            <div class="filter-action" >
              <n-select v-model:value="selectedUnofficialPositions" filterable clearable multiple @update:value="filterRecords(false)" placeholder="សូមជ្រើសរើស ឋានៈស្មើ" :options="optionPositions" />
            </div>
            <div class="filter-action" >
              <n-select v-model:value="selectedEducationLevels" filterable clearable multiple @update:value="filterRecords(false)" placeholder="សូមជ្រើសរើស កម្រិតអប់រំ" :options="optionEducationLevels" />
            </div>
            <div class="filter-action" >
            ...
            <div class="filter-action" >
              <n-select v-model:value="selectedOrganizations" filterable clearable multiple @update:value="filterRecords(false)" placeholder="សូមជ្រើសរើស ស្ថាប័ន / អង្គភាព" :options="optionOrganizations" />
            </div>
          </div>
        </div>
      </Transition>
        ...
        <table v-if="Array.isArray( table.records.matched ) && table.records.matched.length > 0 "
          ...
          <thead >
            <tr class="otc-header-row">
              ...
              <th class="text-left" >មុខតំណែង</th>
              <th class="text-left" >គោត្តនាម និងនាម</th>
              <th class="text-left" >ភេទ</th>
              <th class="text-left" >ថ្ងៃខែឆ្នាំកណើត</th>
              <th class="text-left" >ថ្ងៃខែឆ្នាំចូលបម្រើការងារ</th>
              <th class="text-left" >ថ្ងៃខែឆ្នាំតាំងស៊ប់</th>
              <th class="text-left" >ក្របខ័ណ្ឌ</th>
              <th class="text-left" >កម្រិតវប្បធម៌</th>
              <th class="text-left" >ផ្សេងៗ</th>
              <th class="w-12"></th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(record, index) in table.records.matched" :key='index' 
              ...
              <td class="text-left" v-html="highlightMultipleTerms( record.code , table.pagination.search )" ></td>
              <td class="text-left" >{{ record.current_job.organization_structure_position.position.name }}</td>
              <td class="text-left" v-html="(record.countesy != null ? record.countesy.name + '<br/>' : '') +
                ( highlightMultipleTerms( record.people.lastname , table.pagination.search ) + ' ' + highlightMultipleTerms( record.people.firstname , table.pagination.search ) )" 
                ></td>
              <td class="text-left" >{{ record.people.gender == 1 ? 'ប្រុស' : 'ស្រី' }}</td>
              <td class="text-left" >{{ dateFormat( new Date( record.people.dob ) , 'dd-mm-yyyy' ) }}</td>
              <td class="text-left" >{{ dateFormat( new Date( record.unofficial_date ) , 'dd-mm-yyyy' ) }}</td>
              <td class="text-left" >{{ dateFormat( new Date( record.official_date ) , 'dd-mm-yyyy' ) }}</td>
              <td class="text-left" >{{ record.rank != null ? record.rank.ank + ' - ' + record.rank.prefix : '' }}</td>
  ...
  setup(){
    ...
    function getRecords(){
      ...
      store.dispatch(model.name+'/list',{
        ...
        educatoin_levels: selectedEducationLevels.value,
    ...
    function getPositions(){
      store.dispatch('position/structurePosition',{
        ...
        console.log( store.getters['position/getRecords'] )
      }).catch(err =>{
        notify.error({
          title: 'អានតំណែង' ,
    ...
    function getOrganizations(){
      store.dispatch('organization/organizationStructure',{
        ...
        console.log( store.getters['organization/getRecords'] )
      }).catch(err =>{
        notify.error({
          title: 'អានអង្គភាព' ,
    ...
    const optionEducationLevels = computed( () => {
      let educationLevels = Array.isArray(store.getters['certificate/getRecords']) && store.getters['certificate/getRecords'].length > 0 ? store.getters['certificate/getRecords'] : []
      educationLevels = educationLevels.map((ec) => {
        return { label : ec.education_level + " - " + ec.name , value : ec.id }
      })
      return educationLevels
    })
    function getCertificates(){
      store.dispatch('certificate/group',{
        page: 1 ,
        perPage: 1000 ,
        search: ''
      }).then(res=>{
    ...
    const optionRanks = computed( () => {
      let ranks = Array.isArray(store.getters['rank/records']?.all) && store.getters['rank/records']?.all?.length > 0 ? store.getters['rank/records']?.all : []
      ranks = ranks.map((r) => {
        return { label : r.ank + " - " + r.prefix , value : r.id }
      })
      return ranks
    })
    function getRanks(){
    ...
    function getTermsMultipleDelimiters(sentence, delimiters = [' ', ',', ';', '|']) {
      ...
      const pattern = delimiters.map(d => 
        d.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') // Escape special characters
      ).join('|');
...
.vcb-filters-panel .filter-actions .filter-action {
  @apply p-2 m-2 bg-white rounded-lg shadow border border-gray-300 w-1/4 sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4 ;
}
.highlighted-term {
  @apply bg-yellow-100 ;
}
...
.otc-horizontal-filters-panel .filter-actions .filter-action {
  @apply p-1 bg-white w-full sm:w-1/2 md:w-1/4 lg:w-1/5 xl:w-1/6 ;
}