        ...
        >
          ...
          >
            <div class="w-full font-moul pb-2 text-lg">
              {{
                createMode
                  ? $route.name === "OfficerOthersCreate"
                    ? "បញ្ចូលទិន្នន័យមន្ត្រីផ្សេងៗ"
                    : $route.query.officerType === "politician"
                      ? "បញ្ចូលទិន្នន័យមន្ត្រីនយោបាយ"
                      : $route.query.officerType === "admin_official"
                        ? "បញ្ចូលទិន្នន័យមន្ត្រីមុខងារសាធារណៈ"
                        : "បញ្ចូលទិន្នន័យមន្ត្រីថ្មី"
                  : $route.name === "OfficerOthersCreate"
            ...
            <div class="flex flex-1 justify-end items-center gap-1 pb-2">
              ...
              >
              <button
                v-if="createMode && !officer?.id"
                class="w-24 bg-gray-50 py-2 px-6 border border-gray-300 rounded hover:bg-gray-100 duration-500 hover:text-gray-700"
...
export default {
  ...
  setup(props) {
    ...
    function createOfficerType() {
      const rawOfficerType = Array.isArray(route.query?.officerType)
        ? route.query.officerType[0]
        : route.query?.officerType;
      const queryOfficerType = String(rawOfficerType || "").trim();

      if (route.name === "OfficerOthersCreate") {
        return "contracted_officer";
      }

      if (
        queryOfficerType === "politician" ||
        queryOfficerType === "admin_official"
      ) {
        return queryOfficerType;
      }

      return "admin_official";
    }

    function createEmptyOfficerRecord() {
      return {
        id: null,
        public_key: "",
        code: "",
        ...
        salary_rank: "",
        officer_type: "",
        additional_officer_type: createOfficerType(),
        organization: null,
        position: null,
        countesy: null,
        rank: {
    ...
    async function initializeProfile() {
      ...
      if (createMode.value === true) {
        officer.value = createEmptyOfficerRecord();
        additional_officer_type.value =
          additional_officer_types.value.find(
            (aot) => aot.key === officer.value.additional_officer_type,
          ) || additional_officer_types.value[0];
    ...
    function markAllCareerSectionsSaved() {
      ...
      Object.values(sectionRefs).forEach((sectionRef) => {
        ...
        if (typeof sectionRef.markSaved === "function") {
          console.log( "If markSaved() is function then call it. -- markAllCareerSectionSaved")
    ...
    function ensurePersonalSectionCompleted(section) {
      if (section === "personal") return true;
      const personalSection = personalSectionRef.value;
      if (
        !personalSection ||
        typeof personalSection.validateRequiredFields !== "function"
      )
        return true;
      const isValid = personalSection.validateRequiredFields({ silent: false });
      if (!isValid) {
        focusFailedCareerSection("personal", personalSection);
        throw new Error("Personal section incomplete");
      }
      return true;
    }
    ...
    async function saveCareerSection(section, sectionRefs) {
      ...
      if (!sectionRef) return;
      ensurePersonalSectionCompleted(section);
      ...
      if (typeof sectionRef.markSaved === "function") {
        console.log( "If markSaved is function then call it. -- saveCareerSection" )
        sectionRef.markSaved();
        if (careerSectionChanged.hasOwnProperty(section)) {
          careerSectionChanged[section] = false;
        }
    ...
    async function cancelCareerSection(section) {
      ...
      if (typeof sectionRef.markSaved === "function") {
        console.log( "If markSaved is function then call it. -- cancelCareerSection")
        sectionRef.markSaved();
        if (careerSectionChanged.hasOwnProperty(section)) {
          careerSectionChanged[section] = false;
        }
    ...
    async function saveAllCareerSections() {
      ...
      try {
        ...
        const recordId = parseInt(route.params.id || officer.value?.id || 0);
        if (recordId > 0) {
          ...
          // if (createMode.value === true && !route.params.id) {
          //   // router.replace(`/officer/profilecreate/${recordId}`);
          //   router.replace(`/officer/profile/${recordId}`);
          // }
        }
        ...
        if (saveErrorMessage === "Personal section incomplete") {
          message.error(
            "សូមបំពេញព័ត៌មានក្នុង ក. ព័ត៌មានផ្ទាល់ខ្លួន ឱ្យបានគ្រប់មុនពេលរក្សាទុក",
          );
        } else if (saveErrorMessage === "Failed to save section: krobKhan") {
          message.error(
    ...
    function cancelCreateMode() {
      if (createMode.value !== true) return;
      router.push("/officer");
    }
