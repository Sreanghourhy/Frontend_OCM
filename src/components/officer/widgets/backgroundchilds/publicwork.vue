  ...
  <div class="mb-4">
    <div class="flex items-center justify-between gap-3 mb-4">
      <div class="font-moul text-[#1E3A8A] text-[15px] text-left">
        ច.១.១. ប្រវត្តិក្នុងវិស័យសាធារណៈ
      </div>
      <button
        type="button"
    ...
    <section class="bg-white border border-[#D8DEE8] rounded-sm">
            ...
            >
              ...
              <td class="p-2 border-b border-[#E5E7EB] align-top">
                ...
                <input
                  ...
                  type="text"
                  autocomplete="off"
                  placeholder="បញ្ចូលនាយកដ្ឋាន-អង្គភាព"
              ...
              <td class="p-2 border-b border-[#E5E7EB] text-center">
                <row-document-action-bar
                  :input-id="`publicwork-document-${row._key}`"
                  ...
                  @remove="removeRow(indexOfRow(row))"
                />
              </td>
            </tr>
      ...
      >
        សូមបញ្ចាក់ព័ត៌មានមន្ត្រីជាមុនសិន
      </div>
      <pdf-preview
        :model="model"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function createTextOption(label) {
    ...
    const subOrganizationOptions = computed(() => {
      const records = store.getters['organization/getRecords']
      return buildTextOptions(records, [
        'name_kh',
        'name',
        'title',
        'organization_name'
    ...
    const model = reactive({
      ...
      title: 'ប្រវត្តិក្នុងវិស័យសាធារណៈ'
    })
    ...
    function hydrateRows() {
      ...
      rows.value = source
        .filter((job) => parseInt(job?.sector || 0) === 0)
        .map((job) => ({
    ...
    function indexOfRow(row) {
      return rows.value.findIndex((record) => record?._key === row?._key)
    }
    ...
    function resetSubOrganizationValue(row) {
      row.sub_organization = shouldUseStructuredFields(row) ? [] : ''
    }

    function normalizeSubOrganizationSelection(row) {
    ...
    function subOrganizationSelectOptions(currentValue) {
      return textSelectOptions(subOrganizationOptions.value, currentValue)
    }

    function positionSelectOptions(currentValue) {
      return textSelectOptions(positionOptions.value, currentValue)
    }
    ...
    function onOrganizationChange(row) {
      resetSubOrganizationValue(row)
      row.position = ''
    }

    function onSubOrganizationChange(row) {
      normalizeSubOrganizationSelection(row)
      row.position = ''
    }

    const organizationFilterOptions = computed(() => (
      organizationSelectOptions(organizationFilter.value)
    ))

    const filteredRows = computed(() => {
      const target = String(organizationFilter.value ?? '').trim()
      if (!target) return rows.value
      return rows.value.filter(
        (row) => String(row?.organization ?? '').trim() === target
      )
    })

    watch(
      () => props.officer?.job_backgrounds,
      hydrateRows,
