<template>
    <div class="pt-4 " :class="['mb-10', sectionEdited ? 'section-highlight' : '']">
        <div class="flex items-center justify-between gap-3 mb-1">
            <div class="w-full text-left text-[15px] text-[#1E3A8A] pb-2 font-moul">ឆ.២-ការដាក់វិន័យ</div>
            <button
                type="button"
        ...
        <section class="bg-white border border-[#D8DEE8] border-t-0 rounded-b-sm overflow-hidden">
            <div v-if="officer != undefined" class="overflow-x-auto bg-white">
                <table class="w-full min-w-[980px]">
                    <thead>
                        <tr>
                            <th class="bg-[#E5EAF2] text-[#2C3E50] font-btb-black text-[13px] p-2 border-b border-[#D8DEE8]">លេខលិខិត</th>
                            <th class="bg-[#E5EAF2] text-[#2C3E50] font-btb-black text-[13px] p-2 border-b border-[#D8DEE8]">កាលបរិច្ឆេទ</th>
                            <th class="bg-[#E5EAF2] text-[#2C3E50] font-semibold text-[13px] p-2 border-b border-[#D8DEE8]">ក្រសួង/ស្ថាប័ន/រាជធានី-ខេត្ត</th>
                            <th class="bg-[#E5EAF2] text-[#2C3E50] font-btb-black text-[13px] p-2 border-b border-[#D8DEE8]">បរិយាយ</th>
                            <th class="bg-[#E5EAF2] text-[#2C3E50] font-btb-black text-[13px] p-2 border-b border-[#D8DEE8]">ប្រភេទ</th>
                            <th class="bg-[#E5EAF2] text-[#2C3E50] font-btb-black text-[13px] p-2 border-b border-[#D8DEE8] w-44 text-center">សកម្មភាព</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(row, idx) in rows" :key="row._key" :class="rowEdited(row) ? 'bg-[#FEE2E2]' : 'bg-white'">
                            ...
                            <td class="p-2 border-b border-[#E5E7EB] text-center">
                                <row-document-action-bar
                                    :input-id="`penalty-document-${row._key}`"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const {
            ...
            downloadDocument
        } = useRowDocumentUpload(store, 'officerpenaltyhistory/upload')
        let seed = 0
        ...
        const inputClass = (row, field) => ([
            'field-input w-full text-center border rounded px-2 text-[13px] bg-white',
        ...
        const sectionEdited = computed(() => JSON.stringify(toPayload()) !== savedSnapshot.value)

        const notifyDirty = () => {
            emit('changed', sectionEdited.value)
        }
