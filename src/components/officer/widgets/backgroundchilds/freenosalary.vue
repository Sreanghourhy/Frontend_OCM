  ...
  <div class="mb-10">
    <div class="flex items-center justify-between gap-3 mb-4">
      <div class="font-moul text-[#1E3A8A] text-[15px] text-left">
        ច.៥.ស្ថានភាពស្ថិតនៅក្នុងភាពទំនេរគ្មានបៀវត្ស (សូមបំពេញតាមលំដាប់ ពីថ្មីទៅចាស់)
      </div>
      <button
        type="button"
      ...
      <div v-if="officer != undefined" class="overflow-x-auto">
        ...
        <thead>
          <tr>
            ...
            <th class="bg-[#E5EAF2] text-[#2C3E50] font-semibold text-[13px] p-2 border-b border-[#D8DEE8]">ចំនួន(ខែ/ឆ្នាំ)</th>
        ...
        <tbody>
          <tr v-for="(row, index) in rows" :key="row._key" :class="rowEdited(row) ? 'bg-[#FEE2E2]' : 'bg-white'">
            ...
            <td class="p-2 border-b border-[#E5E7EB]">
              <input
                v-if="showMonthInput(row)"
                :ref="(el) => setMonthInputRef(row._key, el)"
                v-model="row.total_months"
                type="number"
                min="0"
                class="w-full border text-center  border-[#D8DEE8] rounded px-2 py-1.5 text-[13px] bg-white"
                @focus="startMonthEditing(row)"
                @blur="stopMonthEditing(row)"
                @keyup.enter="stopMonthEditing(row)"
              />
              <button
                v-else
                type="button"
                class="w-full border text-center border-[#D8DEE8] rounded px-2 py-1.5 text-[13px] bg-white hover:border-[#93C5FD] duration-200"
                @click="startMonthEditing(row)"
              >
                {{ formatKhmerMonthDuration(row.total_months) }}
              </button>
            </td>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const editingMonthRowKey = ref(null)
    const monthInputRefs = ref({})
    ...
    function setMonthInputRef(key, el) {
      if (el) {
        monthInputRefs.value[key] = el
      } else {
        delete monthInputRefs.value[key]
      }
    }

    async function startMonthEditing(row) {
      editingMonthRowKey.value = row?._key || null
      await nextTick()
      const input = monthInputRefs.value[row?._key]
      if (input?.focus) input.focus()
      if (input?.select) input.select()
    }

    function stopMonthEditing(row) {
      if (!row || editingMonthRowKey.value === row._key) {
        editingMonthRowKey.value = null
      }
    }

    function showMonthInput(row) {
      return (
        !row
        || parseInt(row?.id || 0) <= 0
        || rowEdited(row)
        || editingMonthRowKey.value === row._key
      )
    }
    ...
    function removeRow(index) {
      ...
      if (editingMonthRowKey.value === row?._key) {
        editingMonthRowKey.value = null
      }
