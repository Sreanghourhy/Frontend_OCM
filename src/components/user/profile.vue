<template>
  <div class="profile-layout absolute left-0 top-12 right-0 bottom-0">
    <div class="system-layout h-full overflow-auto p-4">
    <div class="grid grid-flow-row gap-2 p-4 md:grid-flow-col md:grid-cols-10 lg:grid-cols-12 rounded-lg shadow border border-gray-200 " >
      <div class="bg-white p-4 md:col-span-3 lg:col-span-3 xl:col-span-2 2xl:col-span-2 border-r border-gray-200" >
      ...
      <Transition name="slide-fade" >
        <div v-if="transitionHelper" class="relative bg-white p-4 md:col-span-7  lg:col-span-9" >
    ...
    setup(){
      ...
      onMounted( () => {
        if( isAuth() ) {
          ...
          officerDate.value = user.value.officer != null 
            ? (
              user.value.officer.official_date != undefined && user.value.officer.official_date.length > 0
                ? ( new Date( user.value.officer.official_date ) ).getTime() 
                : ( new Date() ).getTime() 
            )
            : null
      ...
      function saveAccount(){
        ...
        if( 
          ...
          notify.info({
            title: 'រក្សារទុកព័ត៌មាន' ,
            content: 'សូមបំពេញព័ត៌មានលេខទូរស័ព្ទ',
            duration: 1000
          })
          return false
        }
