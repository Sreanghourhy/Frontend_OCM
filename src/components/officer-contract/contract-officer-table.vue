...
const CONTRACT_OFFICER_TYPES = ['admin_unofficial']

const model = reactive({
  name: 'officer',
  module: 'officers',
  title: 'មន្ត្រីជាប់កិច្ចសន្យា',
})
...
const COLUMN_VISIBILITY_STORAGE_KEY = 'ocm.hr.officer.contract.listing.columnVisibility.v1'
...
function getRecords () {
  ...
  const requestParams = {
    ...
    ids: peopleIds.value,
    officer_types: CONTRACT_OFFICER_TYPES,
...
function openCreateProfileDraftPage () {
  router.push('/officer-contract/create')
}

function goToOfficerProfile (id) {
  router.push(`/officer-contract/profile/${id}`)
}

const printProfileModal = reactive({ show: false, url: '' })

function goToOfficerPrintProfile (id) {
  printProfileModal.url = `${window.location.origin}/#/officercontract/printprofile/${id}`
